...
def generate_cpp(model_name, dim, u_dim, g, jordan, eat, c, dc, aabb, d_aabb, vr, if_var, if_set, a_str, b_str, c_str, cmp_J_str, cmp_P_str, cmp_exp) :
  code_str = str(
    ...
    "typedef StateSpace<Type,SYS_N,SYS_P,SYS_Q,%sClosedExpm> %sSS;\n" % (model_name,model_name) +
    ...
    'Eigen::Matrix<Type,2*SYS_N,2*SYS_N> operator()(Type t) const\n'
    '{\n'
    '  Eigen::Matrix<Type,2*SYS_N,2*SYS_N> eAt;\n'
    '  %s\n' % cmp_exp +
    '  return eAt;\n'
    '}\n'
    '};\n'
    '//typedef StateSpace<Type,2*SYS_N,SYS_P,SYS_Q> %sSSComposite;\n' % model_name +
    'typedef StateSpace<Type,2*SYS_N,SYS_P,SYS_Q,%sCmpClosedExpm> %sSSComposite;\n' % (model_name,model_name) +
